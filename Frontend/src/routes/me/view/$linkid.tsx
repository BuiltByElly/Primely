...
function RouteComponent() {
  ...
  const datum = links?.filter((d) => d.id === Number(params.linkid))[0];
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col gap-3 mb-5">
            ...
            <p className="flex gap-2 items-center">
              ...
              <button
                className={`text-sm p-1.5 transition-colors rounded-lg ${hasCopied ? "bg-green-500/20" : "bg-foreground/20 hover:bg-foreground/30"}`}
                onClick={async () => {
                  await navigator.clipboard.writeText(
                    datum.short_code
                      ? `${window.location.hostname}/r/${datum.short_code}`
                      : "",
                  );
                  setHasCopied(true);
